...
{
    public:
    Second(std::shared_ptr<Scope>& s,const std::vector<preToken>& v, BEnd b) : scope(s), preTokens(v) , backEnd(b) {};
    ...
    std::vector<preToken> makeParam(std::vector<preToken>& incValue, std::vector<preToken>& names, std::unordered_map<std::string, Variable>& k);
    ...
    std::shared_ptr<valueNode> startFold(const std::shared_ptr<operatorNode>& v);
    ...
    void functionHandling(std::vector<preToken>& b, std::shared_ptr<NFunction>& n);
    std::shared_ptr<valueNode> valuePasser;
    parseLogic logic;
