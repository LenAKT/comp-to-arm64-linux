...
void backEnd::start(){
    ...
    for(auto i : *Ir){
        ...
        case Instruction::Arithma:
                ArithmaFunction(std::static_pointer_cast<ArIr>(i));
            break;
...
void backEnd::runner(std::shared_ptr<IrNode> i){
    ...
    for (int i = 0; i < s->params.size(); i++)
    {
        ...
        case TokenCategory::VARIABLE:
            outFile << "mov x" << i << ", x" << scope->Cvars[s->params[i].stringValue]->reg;
        default:
            break;
        }
    }
...
void backEnd::ArithmaFunction(std::shared_ptr<ArIr> ir){
    VarType type = scope->Cvars[ir->caller]->var.type;
    bool isGood = false;
    ...
    auto check = [&](auto node) -> bool{
        if (node->type == valueType::Varible)
        {
            ...
            if (type == VarType::Int)
            {
                if (int i = 0){trueVal = &std::static_pointer_cast<intNode>(node)->value;}
    ...
    if (isGood)
    {
        ...
        case VarType::Int:{
                ...
                outFile << b->second << " w" << scope->Cvars[ir->caller]->reg << ", w"<< ((trueVal != nullptr) ? *trueVal : leftReg)
